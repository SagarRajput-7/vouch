...
## 2026-09-15: promoteToAccount retires the target's other active workspace

**Decision.** `workspacesRepo.promoteToAccount` now runs in a transaction that first soft-deletes any other workspace the target user already owns with `deleted_at is null`, then reassigns ownership of the promoted workspace, so it becomes the target's sole active workspace.
**Alternatives.** Leave the plain reassigning `UPDATE` as originally written; reject the promotion with a thrown error when the target already owns an active workspace; delete (not soft-delete) the target's prior workspace outright.
**Reasoning.** The code review that added `workspaces_owner_active_idx` (one active workspace per owner, enforced by a partial unique index) also asked for an integration test that promotes workspace A to a user who already has their own active guest workspace from a separate `startGuestSession()` call. Run against the reassigning `UPDATE` alone, that exact scenario throws `duplicate key value violates unique constraint "workspaces_owner_active_idx"`, since the update would leave the target owning two active rows at once. The same shape of conflict is reachable for real through the `anonymous` plugin's `onLinkAccount` hook whenever a user links a second anonymous session to an account that already has one, for example a second browser or device. Soft-deleting the target's prior workspace keeps the one-active-workspace-per-owner invariant the index now enforces, keeps the promoted workspace's row and id stable for anything already holding a reference to it, and preserves the retired workspace's data instead of destroying it. The transaction keeps the retire-then-reassign pair atomic, closing the same kind of race the unique index and `onConflictDoNothing` already close for `findOrCreateForUser`.
**Cut.** A real merge of the two workspaces' contents (documents, invoices, and so on). Nothing in the schema yet models one workspace absorbing another's rows, and this task's scope is bootstrapping sessions and workspaces, not account merging.
