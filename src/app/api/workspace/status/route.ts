...
export const GET = handle(async (request) => {
  ...
  // queuedJobs in the response is scoped to this workspace: every workspace-scoped value in
  // the API must derive from the session. The drain trigger below still consults the global
  // queue (never returned to the client) because the drain it kicks off is a shared,
  // best-effort safety net that processes any workspace's jobs, not just this caller's.
  const queuedJobs = await jobsRepo.countQueued(session.workspaceId);
  const globalQueued = await jobsRepo.countQueued();
  if (globalQueued > 0 || inFlight.length > 0) scheduleDrain("status-poll");
